...
namespace EventSourcing.Sample.Model.Forum
{
    public class Forum : AggregateRoot<Guid>
    {
        ...
        {
            OnEvent(new ForumCreated(Id, name, new ForumState()));
        }

        [DomainHandler("ForumId", GetWithLock=true)]
        private void ChangeState(ThreadCreated threadCreated)
        {
            ...
            OnEvent(new ForumStateChanged(Id, state));
        }
        [DomainHandler("Thread:ThreadId", "Forum:ForumId", GetWithLock = true)]
